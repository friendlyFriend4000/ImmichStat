<script lang="ts">
	...
	function updateChartData(data: ServerStats) {
		...
		chartOptions.plotOptions.pie.donut.labels.total.formatter = () => {
			switch (mediaType) {
				...
				case 'videos':
					return data.videos;
				case 'usage':
					return convertBytes(data.usage);
				default:
					throw new Error(`Invalid mediaType: ${mediaType}`);
			}
		};
		...
		chartOptions.dataLabels.formatter = (value) => {
			switch (mediaType) {
				case 'photos':
					return ((value / 100) * data.photos).toFixed(0);
				case 'videos':
					return ((value / 100) * data.videos).toFixed(0);
				case 'usage':
					return convertBytes((value / 100) * data.usage);
				default:
					throw new Error(`Invalid mediaType: ${mediaType}`);
			}
		};
		...
		chartOptions.tooltip.y.formatter = (value) => {
			const total =
				mediaType === 'photos' ? data.photos : mediaType === 'videos' ? data.videos : data.usage;
